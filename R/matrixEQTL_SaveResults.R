...
trans.cross.chr <- ifelse(tolower(trimws(args[6]))=="yes",".CrossChr","")
save.csv.cis <- ifelse(tolower(trimws(args[7]))=="yes",T,F)
save.csv.trans <- ifelse(tolower(trimws(args[8]))=="yes",T,F)
...
if(!file.exists(merge_file)){
  for (i in 1:22) {
    file_ <- paste0(FilePrefix,".matrixEQTL.chr",i,"/",paste0(FilePrefix,".matrixEQTL.chr",i,".RData"))
